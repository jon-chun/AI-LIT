...
filename_out = "book_proust_en_swans-way_moncrieff_sentence_clean.txt"
...
FULLPATH_SEGMENTS_FILE_OUT = os.path.join('..', 'data', 'step1_segments', subdir_title, filename_out)
...
def process_segments_file(textfile_str, language='en'):
    """
    ...
    try:
        ...
        start_time = time.time()
        sentences_regex_list = segment_text_regex(textfile_str)
        print(f"The execution took {time.time() - start_time} seconds.")
...
def main():
    ...
    sentences_out_list = process_segments_file(file_text_str)
    print(f"  RETURN from process_segment_file with len(sentences_out_list): {len(sentences_out_list)}")
    # 
    write_sentences_to_file(FULLPATH_SEGMENTS_FILE_OUT, sentences_out_list)
